...
public class MyAgent extends Agent {
    ...
    /**
     * Constructs a new agent, giving it the game and telling it whether it is Red or Blue.
     * @param game The game the agent will be playing.
     * @param iAmRed True if the agent is Red, False if the agent is Blue.
     */
    ...
    public void move() {
    	if (ICanWin() > -1) {
    		...
    		myGame.moveOnColumn(ICanWin(), iAmRed);
    		...
    		myGame.moveOnColumn(TheyCanWin(0, 0, false), iAmRed);
    	} else {
    		// If neither me nor the opponent have the chance to win, then I just make a brilliant move.
    		myGame.moveOnColumn(brilliantMove(), iAmRed);
    	}
    }

    public void move(int x, int y) {
        int col = (x - 10) / 80;
        myGame.moveOnColumn(col, iAmRed);
    }
    ...
    public int randomMove() {
        ...
        while (column.getLowestEmptyIndex(column) == -1) {
    ...
    public int brilliantMove() {    
    	...
    	for (int i = 0; i < column; i++) {
    		if (myGame.getLowestEmptyIndex(i) > -1 && !dangerous.contains(i)) {
    ...
    public int anyoneCanWin(boolean myColor, int colDef, int rowDef, boolean immediate) {
    	...
    	for (int i = colDef; i < myGame.getColumnCount(); i++) {
    		int j = myGame.getLowestEmptyIndex(i) - rowDef;
    		if (j > -1) {    			
    ...
    public String getName() {    
        return "My Agent";
        // Welcome to Connect 4! My Agent (Red) vs. Welcome to Connect 4! My Agent (Blue) ?????????????????????? 
    }
     ...
     * A route has two ends. If an end is the lowest empty slot in that column, then it is ready to be filled.
     * @param columnIndex the index of the column, in which a token might be placed
     * @param color true if red, false, if blue
    ...
    private Integer[] routeLength(int columnIndex, boolean color, int pattern, int rowDef) {
    	...
    	if (pattern == 1) { // vertical
    		y = 1;
    	} else if (pattern == 2) { // horizontal
    		x = 1;
    	} else if (pattern == 3) { // descending slope
    		x = 1;
    		y = 1;
	    	...
	    	while (!finished) {
	    		...
	    		if (!outOfBound(columnIndex, deltaX, deltaY, rowDef) && getIsFilled(columnIndex, deltaX, deltaY, rowDef) &&
	    			...
	    			length++;
	    				   ...
	    				   myGame.getLowestEmptyIndex(columnIndex + deltaX) == myGame.getLowestEmptyIndex(columnIndex) + deltaY - rowDef) {
    ...
    private boolean outOfBound(int columnIndex, int x , int y, int rowDef) {
    	...
    	int currentY = myGame.getLowestEmptyIndex(columnIndex) - rowDef;
    ...
    /*
    public boolean getIsCurrentFilled(int x, int y) { // according to mouse location
